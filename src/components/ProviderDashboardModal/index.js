...
const ProviderDashboardModal = ({ show, onClose, provider }) => {
  ...
  useEffect(() => {
    if (show) {
      ...
      setPurchasesPage(1);
      setSalesPage(1);
      setSummary({ totalCompras: 0, numCompras: 0, totalVentas: 0, numVentas: 0, bestSeller: null });
  ...
  useEffect(() => {
    ...
    const doFetch = async () => {
      setLoading(true);
      try {
        const result = await fetchProviderSummary(provider.IdProveedor, {
          from: dateRange.from,
          to: dateRange.to,
          showNoe,
        });
    ...
    try {
      ...
      const docDef = {
        content: [
          ...
          { text: 'CALLE ILUSTRES PROCERES LOCAL NRO S/N SECTOR CENTRO ALTAGRACIA DE ORITUCO DE ORITUCO ZONA POSTAL 2320.', style: 'header' },
          ...
          { text: `Proveedor: ${provider.Empresa}`, style: 'subheader', margin: [0, 0, 0, 12] },
          {
            ...
            table: {
              ...
              body: [
                ...
                ...rows,
                ['', '', { text: 'Total', bold: true }, { text: formatCurrency(detail.total), bold: true }],
              ],
            },
          },
        ],
  ...
  const handlePrintAllPurchases = async () => {
    try {
      ...
      for (const inv of invoices) {
        ...
        tables.push(
          { text: `Factura: ${detail.idFactura} — ${DateTime.fromISO(detail.fecha).toFormat('dd/MM/yyyy')}`, style: 'invoiceTitle', margin: [0, 14, 0, 4] },
          {
            style: 'table',
            table: {
              widths: ['*', 'auto', 'auto', 'auto'],
      ...
      const docDef = {
        content: [
          ...
          { text: `Período: ${DateTime.fromISO(dateRange.from).toFormat('dd/MM/yyyy')} — ${DateTime.fromISO(dateRange.to).toFormat('dd/MM/yyyy')}`, style: 'subheader', margin: [0, 0, 0, 10] },
          ...tables,
        ],
        styles: {
          header: { alignment: 'center', fontSize: 10 },
          subheader: { alignment: 'center', fontSize: 9, margin: [0, 2, 0, 2] },
          ...
          <table className='provider-table'>
            <thead>
              <tr>
                ...
                <th className='text-center' style={{ width: 50 }}>Imprimir</th>
              </tr>
            </thead>
            <tbody>
              {purchasesData.data.length > 0 ? (
                purchasesData.data.map((purchase) => (
                  ...
                  >
                    ...
                    <td>{DateTime.fromISO(purchase.fecha).toFormat('dd MMM yyyy', { locale: 'es' })}</td>
                    <td className='text-end provider-amount'>{formatCurrency(purchase.monto)}</td>
                    <td className='text-center'>
                      <button
                        className='btn btn-sm btn-outline-light'
                        onClick={(e) => handlePrintPurchase(purchase, e)}
                  ...
                  <tr>
                    <td colSpan={4}>
                      <div className='provider-empty-state'>
                        ...
                        <span>Sin compras en este período</span>
                      </div>
                    </td>
                  </tr>
                )
              ...
              {salesData.data.length > 0 ? (
                salesData.data.map((sale, index) => (
                  <tr key={index}>
                    ...
                    <td className='text-end provider-amount'>{formatCurrency(sale.monto)}</td>
                  </tr>
                ))
              ) : (
                !salesLoading && (
  ...
  return (
    <>
      <Modal show={show} size='xl' onHide={onClose} backdrop='static' scrollable className='provider-dashboard-modal'>
